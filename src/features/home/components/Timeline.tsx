...
export const Timeline: React.FC<TimelineProps> = ({ editionId }) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 glass-card rounded-3xl p-8 w-full relative z-10">
  ...
  if (displaySchedule.length === 0) {
    return (
      <div className="w-full text-center py-16 glass-card rounded-3xl p-8 flex flex-col items-center justify-center gap-4 relative z-10">
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p className={`text-xs leading-relaxed mb-4 line-clamp-3 ${isKeynote ? 'text-white/80' : 'text-light/85'}`}>
                                {act.description}
                              </p>
                            </div>

                            {speaker && (
                              <div className={`flex items-center gap-3 pt-3 border-t mt-auto ${isKeynote ? 'border-white/10' : 'border-accent/10'}`}>
                                <img
                                ...
                                <div className="flex flex-col overflow-hidden">
                                  <span className={`text-xs font-semibold truncate ${isKeynote ? 'text-white' : 'text-light'}`}>{speaker.name}</span>
                                  <span className={`text-[10px] truncate ${isKeynote ? 'text-[#fcd34d]' : 'text-accent'}`}>{speaker.specialty}</span>
                                </div>
                              </div>
                            )}
                          </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <p className={`text-xs leading-relaxed mb-4 ${isKeynote ? 'text-white/80' : 'text-light/85'}`}>
                            {act.description}
                          </p>

                          {speaker && (
                            <div className={`flex items-center gap-3 pt-3 border-t ${isKeynote ? 'border-white/10' : 'border-accent/10'}`}>
                              <img
                                src={speaker.photoUrl}
                                alt={speaker.name}
                              ...
                              <div className="flex flex-col">
                                <span className={`text-xs font-bold ${isKeynote ? 'text-white' : 'text-light'}`}>{speaker.name}</span>
                                <span className={`text-[10px] ${isKeynote ? 'text-[#fcd34d]' : 'text-accent'}`}>{speaker.specialty}</span>
                              </div>
                            </div>
                          )}
                        </div>
